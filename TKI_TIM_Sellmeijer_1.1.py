...
als er een sondering beschikbaar is.
...
######################################
        dfn = df.iloc[: , [1]].copy()   
        dfn = pd.concat([df, pd.DataFrame.from_records([{'npor':0.25},])], ignore_index=True)
...
######## Monte Carlo simulatie
        ...
        tfont = {'fontname':'monospace', 'fontsize':'9'}
        ...
        tfont = {'fontname':'monospace','size':8, 'color':'blue'}
        ax[0,0].text(185, ymin-4,'Monte_Carlo simulatie, [min, mediaan, max]', **tfont)
        ax[0,0].text(185, ymin-6,'MC_sim D               '+ str([diktesell-5, diktesell, diktesell+5]), **tfont)
        ax[0,0].text(185, ymin-7,'MC_sim L               '+ str([l_entry_exit-5, l_entry_exit, l_entry_exit+5]), **tfont)
        ax[0,0].text(185, ymin-8,'MC_sim RD              '+ str([0.6, 0.7,0.8]), **tfont)
        ax[0,0].text(185, ymin-9,'MC_sim d70             '+ str([int(df.d70.quantile(0.14)*1e6), int(d70_gem), int(df.d70.quantile(0.95)*1e6)]), **tfont)
        ax[0,0].text(185, ymin-10,'MC_sim k               '+ str([1, int(df.k.quantile(0.5)), int(df.k.quantile(0.995))]), **tfont)
        ax[0,0].text(185, ymin-11,'MC_sim rolweerstand    '+ str([36,37,38]), **tfont)
        ax[0,0].text(185, ymin-12,'MC_sim temp            '+ str([5,10,15]), **tfont)
        ax[0,0].text(185, ymin-14,'Percentage MC_sim >Lpipe ' +str(round(100-(len(xxp[xxp < l_entry_exit])/N)*100,1)), **tfont)

        ax[0,0].text(260, ymin-6,' [m]' , **tfont)
        ax[0,0].text(260, ymin-7,' [m]' , **tfont)
        ax[0,0].text(260, ymin-8,' [-]' , **tfont)
        ax[0,0].text(260, ymin-9,' [mu]', **tfont)
        ax[0,0].text(260, ymin-10,' [m/dag]', **tfont)
        ax[0,0].text(260, ymin-11,' [graden]', **tfont)
        ax[0,0].text(260, ymin-12,' [graden Celsius]', **tfont)
        ax[0,0].text(260, ymin-14,' [%]', **tfont)
